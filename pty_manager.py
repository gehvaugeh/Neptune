...
class PTYManager:
    ...
    async def _queue_worker(self, uid: int, pty: BasePTY):
        ...
        # Wrapped broadcast with batching for output
        output_buffer = ""
        last_broadcast = 0

        async def batched_broadcast(msg):
            nonlocal output_buffer, last_broadcast
            if msg.get("type") == "output" and msg.get("pty_uid") == uid:
                output_buffer += msg.get("data", "")
                now = asyncio.get_running_loop().time()
                if now - last_broadcast > 0.02: # 20ms batching
                    await self.broadcast({"type": "output", "block_id": msg.get("block_id"), "data": output_buffer, "pty_uid": uid})
                    output_buffer = ""
                    last_broadcast = now
            else:
                if output_buffer:
                    # Flush before other messages
                    # We need to know which block_id to flush for.
                    # Usually it's the current one.
                    if pty.current_block_id:
                        await self.broadcast({"type": "output", "block_id": pty.current_block_id, "data": output_buffer, "pty_uid": uid})
                    output_buffer = ""
                await self.broadcast(msg)

        # Temporarily override pty.broadcast
        original_broadcast = pty.broadcast
        pty.broadcast = batched_broadcast
        ...
        try:
            while True:
                ...
                try:
                    ...
                    # Final flush
                    if output_buffer:
                        await self.broadcast({"type": "output", "block_id": block.get("id"), "data": output_buffer, "pty_uid": uid})
                        output_buffer = ""
                finally:
