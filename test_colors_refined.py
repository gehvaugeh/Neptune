...
def test_hex_colors():
    ...
    # Test cache persistence of error flag
    block._color_error = False
    style_inv_cached = block._get_rich_style(char_inv)
    assert style_inv_cached.bold == True
    assert block._color_error == True

    # Test bright mapping (darkgray -> bright_black)
    char_dg = pyte.screens.Char("G", fg="darkgray", bg="default")
    style_dg = block._get_rich_style(char_dg)
    assert style_dg.color.name == "bright_black"

if __name__ == "__main__":
    try:
        test_hex_colors()
        print("Refined color logic tests PASSED")
    except Exception as e:
        print(f"Refined color logic tests FAILED: {e}")
        import traceback
        traceback.print_exc()
